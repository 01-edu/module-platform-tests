...
tests.push(async ({ page, eq }) => {
  ...
  const content = await page.$$eval('.gossip', nodes =>
...
const getBackground = (nodes, _key) =>
  nodes.map(n => n.style.background || n.style.backgroundColor)
tests.push(async ({ page, eq, rgbToHsl }) => {
  ...
  const lightness = values.map(rgbToHsl).map(([_h, _s, l]) => l)
...
tests.push(async ({ page, eq, rgbToHsl }) => {
  ...
  const values = await page.$$eval('div.gossip', getBackground)
  const lightness = values.map(rgbToHsl).map(([_h, _s, l]) => Math.round(l))
